...
@section('isi')
                    ...
                    <form method="post" action="{{ url('/pegawai/shift/proses-tambah-shift') }}">
                        ...
                        <div class="form-check mb-4">
                            <input name="lock_location" class="form-check-input lock_location" type="checkbox"
                                value="{{ old('lock_location') }}" id="lock_location">
            ...
            <div class="card">
                <div class="card-header">
                    ...
                    <form action="{{ url('/pegawai/shift/' . $karyawan->id) }}">
                        <div class="row">
                            <div class="col-3">
                                <input type="datetime" class="form-control" name="tanggal" placeholder="Tanggal"
                                        ...
                                        <tr>
                                            <td>
                                                <input type="checkbox" class="form-check-input shift-checkbox"
                                                    name="shift_ids[]" value="{{ $sk->id }}">
                                            </td>
                                            <td>{{ ($shift_karyawan->currentpage() - 1) * $shift_karyawan->perpage() + $key + 1 }}.
                                            </td>
                                            <td>{{ $sk->tanggal ?? '-' }}</td>
                                            ...
                                            <td>
                                                <ul class="action">
                                                    ...
                                                    <li class="delete">
                                                        <button type="button" class="border-0 btn-delete-shift"
    ...
    <!-- Hidden form for delete (outside bulkEdit form) -->
    ...
    @push('script')
        <script>
            $(function () {
                ...
                // Delete shift handler - use hidden form
                $(document).on('click', '.btn-delete-shift', function () {
                    var shiftId = $(this).data('id');
                    var userId = $(this).data('user-id');

                    if (confirm('Apakah Anda yakin ingin menghapus shift ini? Data poin kinerja juga akan terhapus.')) {
                        var form = $('#deleteShiftForm');
                        form.attr('action', '/pegawai/delete-shift/' + shiftId);
